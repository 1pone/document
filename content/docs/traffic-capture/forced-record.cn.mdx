...
利用 AREX，你可以通过强制录制一条线上真实的请求，并在测试环境中进行回放验证系统正确性。借助 AREX 强大的 Mock 机制，录制过程中 AREX 将自动对该条真实请求所有的外部依赖进行 Mock，帮你在本地完美还原线上生产数据环境，解决回归测试中**数据构造难、测试用例编写难**的难题。

<Callout title="提示">只有部署了 AREX Agent 的服务接口才能进行强制录制的操作。</Callout>
...
点击新增的 **Mock**，里面是 AREX 录制过程中的所有依赖的数据。左侧是 Mock 到的主接口及外部调用的请求报文，右侧是对应的响应报文。
...
<Callout title="注意">无法对主接口的返回值进行修改，因为主接口的返回值是不 Mock 的，AREX 只会对中间过程中调用的第三方依赖进行 Mock，修改才有效。</Callout>
...
2. 在请求 Header 中添加 Key: arex-record-id，Value: 先前得到的录制 ID。
    ...
    ![回放](../resource/c3.replay.png)

3. 点击 **Send** 发送请求，完成本地回放，返回报文的 Header 中会生成 Replay ID，Key:arex-replay-id，Value:回放 ID。现在可以正常进行 Debug 操作。

    ![回放ID](../resource/c3.force.replayid.png)
